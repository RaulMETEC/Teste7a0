  ...
  if (typeof url === "string" && url.includes("/squads/")) {
    ...
    if (requestCount === 1) {
      ...
      if (remainingSquads.length === 0) {
        console.log("Todas as squads da lista já foram utilizadas.");
        return oldFetch(...args);
      }

      let available = remainingSquads.filter(
        s => getCountry(s) !== lastCountry
      );

      // Se só restarem squads do mesmo país, libera todas
      if (available.length === 0) {
        available = [...remainingSquads];
      }

      forced = available[Math.floor(Math.random() * available.length)];

      // Remove para nunca mais aparecer
      remainingSquads = remainingSquads.filter(s => s !== forced);
    }
    ...
    console.log(
      "FORCED:",
      forced,
      `(${remainingSquads.length} restantes)`
    );
